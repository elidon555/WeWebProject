...
$(function() {
    ...
    */
   $('.filter').select2({
      placeholder: 'Select an item', allowClear: true, ajax: {
         url: 'ajax.php', dataType: 'json', delay: 250, data: function(params) {
            ...
            if (index == 0) {
               ...
               email = $('.filter').eq(0).select2('data');
      ...
      ajax: {
         ...
         data: function(data) {
            ...
            if (window.allowFilter === 1) {
               ...
               let email = filter_s.eq(0).select2('data');
               let phone_number = filter_s.eq(1).select2('data');
               let dateValue = date_s.val();

               (isEmpty(email)) ? email = '' : email = email[0].text;
               (isEmpty(phone_number)) ? phone_number = '' : phone_number = phone_number[0].text;
   ...
   signup_modal_s.on('hidden.bs.modal', function() {
      ...
      /**
       * Reset input fields values,style and error messages
       */
      ...
      //hide image preview of upload file
      $('#blah').hide();
   });
